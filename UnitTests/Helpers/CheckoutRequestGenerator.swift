...
final class CheckoutRequestGenerator {
    func generate(
        ...
        threeDS: Bool? = nil,
        requireEnrolledCard: Bool? = nil,
        requireSuccessfulLiabilityShiftForEnrolledCard: Bool? = nil,
        ...
        if let threeDS = threeDS {
            threeDSecureData = CheckoutRequestData.ThreeDSecure(
                enable: threeDS,
                requireEnrolledCard: requireEnrolledCard ?? threeDS,
                requireSuccessfulLiabilityShiftForEnrolledCard: requireSuccessfulLiabilityShiftForEnrolledCard ?? threeDS
            )
        } else {
        ...
        return CheckoutRequest(content: encodeRequest(data: request))
    }
    
    func generateDonation(options: [Int]? = [1000, 2000, 3000], min: Int? = nil, max: Int? = nil, currency: String = "EUR", rememberMe: Bool = false) -> CheckoutRequest {
    ...
    func generateSubscription(planId: String) -> CheckoutRequest {
        let request = CheckoutRequestData(
            charge: nil,
